...
<section id="widget-grid" class="">
        ...
        <div class="jarviswidget jarviswidget-sortable" id="wid-id-21" data-widget-colorbutton="false" data-widget-editbutton="false" data-widget-custombutton="false" role="widget" style="position: relative; opacity: 1; left: 0px; top: 0px;">
            ...
            <div role="content">
                ...
                <div class="widget-body no-padding">
                    ...
                    <form action="/admin/categories/addCategory" method="post" class="smart-form"  >
    ...
    <article class="col-xs-12 col-sm-12 col-md-12 col-lg-12 sortable-grid ui-sortable">
        ...
        <!-- Widget ID (each widget will need unique ID)-->
        <div class="jarviswidget jarviswidget-sortable" id="wid-id-0" data-widget-editbutton="false" role="widget" style="position: relative; opacity: 1; left: 0px; top: 0px; display: block;" data-widget-attstyle="">
            <!-- widget options:
            usage: <div class="jarviswidget" id="wid-id-0" data-widget-editbutton="false">
                                ...
                                <td class="with-btn-group">
                                    <div class="btn-group m-r-5 m-b-5">
                                        ...
                                        <ul class="dropdown-menu pull-right">
                                            ...
                                            <li><a
                                                    data-id="<?= $cat->id ?>"
                                                    data-title="Ви дійсно хочете видалити цю категорію?"
                                                    data-text="У Вас не буде можливості його відновити"
                                                    class="delete"
                                                    href="/admin/categories/delete/<?= $cat->id ?>">Видалити</a>
                                            </li>
                                        </ul>
                                    </div>
                                </td>
...
<script>
    $(document).ready(function () {
        $('#editModal').on('show.bs.modal', function (event) {
            ...
            var rusname = tr.find('#js-rus-name').text();
            var desc = tr.find('#js-desc').text();
            var decsrus = tr.find('#js-rus-desc').text();
            var cover = tr.find('#js-cover').val();
            ...
            modal.find('#rusname').val(rusname.trim());
            modal.find('#desc').val(desc.trim());
            modal.find('#descrus').val(decsrus.trim());
            if (cover != '') {
                var coverPage = modal.find('#cover').attr("src", cover);
            } else {
                coverPage = modal.find('#cover').attr("src", 'http://dummyimage.com/350x200/');
            }
        });

        $('#editModal_img').on('show.bs.modal', function (event) {
            var button = $(event.relatedTarget); // Button that triggered the modal
            var tr = button.closest('tr');

            var cover = tr.find('#js-cover-big').val();
            console.log(cover);
            var catid = tr.data('id');

            var modal = $('#editModal_img');

            modal.find('#catid').val(catid);

            if (cover != '') {
                var coverPage = modal.find('#cover-big').attr("src", cover);
            } else {
                coverPage = modal.find('#cover').attr("src", 'http://dummyimage.com/350x200/');
            }
        });
